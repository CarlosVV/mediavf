...
namespace MediaVF.Services.Data
{
    ...
    public class DataContext
    {
        ...
        #region StoredProcData

        protected class StoredProcData
        {
            public string ProcName { get; private set; }

            Dictionary<string, Type> _parameters;
            ...
            public StoredProcData(StoredProcDefinition definition)
            {
                ProcName = definition.Name;

                foreach (StoredProcParameter parameter in definition.Parameters)
                    Parameters.Add(parameter.Name, parameter.ParameterType);
            }

            public Dictionary<string, object> GetParameterListForExecution(List<object> parameterValues)
            {
                Dictionary<string, object> parameters = new Dictionary<string, object>();
                for (int i = 0; i < parameterValues.Count; i++)
                {
                    KeyValuePair<string, Type> parameterDefiniton = Parameters.ElementAt(i);
                    if (parameterValues[i] == null)
                        parameters.Add(parameterDefiniton.Key, DBNull.Value);
                    else if (parameterValues[i] != null && parameterValues[i].GetType() == parameterDefiniton.Value)
                        parameters.Add(parameterDefiniton.Key, parameterValues[i]);
                    else
                        throw new InvalidOperationException(string.Format("Parameter value provided is not the correct type. Expected: {0}, Actual: {1}",
                            parameterDefiniton.Value,
                            parameterValues[i].GetType()));
                }
        ...
        Dictionary<string, StoredProcData> _storedProcs;
        protected Dictionary<string, StoredProcData> StoredProcs
        {
            get
            {
                if (_storedProcs == null)
                    _storedProcs = new Dictionary<string, StoredProcData>();
                return _storedProcs;
            }
        }
        ...
        public void UpdateFromConfig(DataContextElement dataContextConfig)
        {
            ...
            List<DataTypeMappingElement> parentTypeMappings =
                dataContextConfig.TypeMappings.Where(typeMapping => !Tables.ContainsKey(typeMapping.ObjectType)).ToList();
            ...
            List<StoredProcDefinition> storedProcDefinitions =
                dataContextConfig.StoredProcs.Where(proc => !StoredProcs.ContainsKey(proc.Name)).ToList();

            if (storedProcDefinitions != null && storedProcDefinitions.Count > 0)
                storedProcDefinitions.ForEach(proc => StoredProcs.Add(proc.Name, new StoredProcData(proc)));
        }
        ...
        public List<T> GetByModuleID<T>(int moduleID, bool loadChildren)
        {
            ...
            criteria.Add("ModuleID", moduleID);

            return Get<T>(criteria, loadChildren);
        }
        ...
        public List<T> Get<T>(string storedProc, List<object> parameters)
        {
            if (StoredProcs.ContainsKey(storedProc))
            {
                StoredProcData storedProcData = StoredProcs[storedProc];

                string sql = SQLBuilder.BuildStoredProcExecute(storedProcData.ProcName, storedProcData.GetParameterListForExecution(parameters));
