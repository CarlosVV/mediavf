...
namespace MediaVF.Services.Core.Data
{
    public static class SQLBuilder
    {
        ...
        public static string BuildSelectByModuleID(string tableName, int moduleID)
        {
            Dictionary<string, object> criteria = new Dictionary<string, object>();
            criteria.Add("ModuleID", moduleID);

            return BuildSelect(tableName, criteria);
        }
        ...
        public static string BuildLinkedTableSelectByModuleID(string table1Name, string table1Column, string table2Name, string table2Column, int moduleID)
        {
            Dictionary<string, object> criteria = new Dictionary<string, object>();
            criteria.Add("ModuleID", moduleID);
...
";
        ...
        public static string BuildInsert(string tableName, string uniqueKey, string idColumn, Dictionary<string, object> columnValues)
        {
            ...
            for (int i = 0; i < columnValues.Count; i++)
            {
                ...
                bool wrapInSingleQuotes = false;
                if (columnValue != null)
                {
                    wrapInSingleQuotes = columnValue.GetType() == typeof(string) || columnValue.GetType() == typeof(DateTime) || columnValue.GetType() == typeof(DateTime?);
                    columnValueText = columnValue.ToString();
                }

                columnValuesText.Append("\t").Append(i > 0 ? "," : " ").Append(wrapInSingleQuotes ? "'" : "").Append(columnValueText).AppendLine(wrapInSingleQuotes ? "'" : "");
            }
